...
import { forwardRef, useState, ChangeEvent, FocusEvent, ReactElement } from 'react';
...
const Input = forwardRef<HTMLInputElement, TextFieldProps>(function CustomizedTextField(
  { type, placeholder, error, errorMessage, value, onChange, onBlur },
  ref,
) {
  const [showPassword, setShowPassword] = useState(false);
  ...
  return type === 'password' ? (
    <StyledFormControl sx={{ m: 1, width: '100%' }} variant="outlined" error={error}>
      <InputLabel htmlFor="outlined-adornment-password" sx={{ backgroundColor: 'white' }}>
        {placeholder}
      </InputLabel>
      <OutlinedInput
        id="outlined-adornment-password"
