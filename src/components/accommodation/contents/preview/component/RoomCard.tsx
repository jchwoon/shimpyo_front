...
export const RoomCard: React.FC<RommCardProps> = ({
  name,
  doubleBed,
  bedroom,
  shower,
  ...
  const roomImage = useRecoilValue(roomImageListState);
    ...
    <div>
      <CustomizedCard onClick={onClick} active={active}>
        <CardMedia
          ...
          image={roomImage[idx][0]}
        />
        <CardContent sx={{ width: '100%' }}>
          <CustomizedTitleRowBox>
            <CustomizedTitleTypography fontFamily="Noto Sans KR">{name}</CustomizedTitleTypography>
          </CustomizedTitleRowBox>

          <div
            style={{ display: 'flex', flexDirection: 'row', marginTop: '15px', marginBottom: '10px', flexWrap: 'wrap' }}
          >
            {bedroom ? (
              <OptionWrapper>
                <MdOutlineBedroomParent size={25} />
                <Typography fontFamily="Noto Sans KR" fontSize="5px">
                  침실 {bedroom}개
                </Typography>
              </OptionWrapper>
            ) : null}
